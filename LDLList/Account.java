// Declaring a package called LDLinkedList.
package LDLList;
...
public class Account{
    ...
    /** The LDLinkedList for posts of the account*/ 
    private LDLinkedList<Post>    posts;
    /** The LDLinkedList for inbox of the account*/ 
    private LDLinkedList<Message> inbox;
    /** The LDLinkedList for outbox of the account*/ 
    private LDLinkedList<Message> outbox;
    /** The LDLinkedList for following of the account*/ 
    private LDLinkedList<Account> following;
    /** The LDLinkedList for followers of the account*/ 
    private LDLinkedList<Account> followers;
    /** The LDLinkedList for blocked accounts of the account*/ 
    private LDLinkedList<Account> blockedAccounts;
    /** The LDLinkedList for history */
    ...
    /**
     * This function returns an LDLinkedList of Post objects.
     * 
     * @return An LDLinkedList of Post objects.
     */
    public LDLinkedList<Post>    getPosts(){
        return posts;
    }
    /**
     * This function returns the inbox LDLinkedList.
     * 
     * @return The inbox LDLinkedList.
     */
    public LDLinkedList<Message> getInbox(){
        return inbox;
    }
    /**
     * This function returns the outbox LDLinkedList.
     * 
     * @return The outbox LDLinkedList.
     */
    public LDLinkedList<Message> getOutbox(){
        return outbox;
    }
    /**
     * This function returns the following LDLinkedList.
     * 
     * @return An LDLinkedList of Account objects.
     */
    public LDLinkedList<Account> getFollowing(){
        return following;
    }
    /**
     * This function returns an LDLinkedList of accounts that are following the account that called this
     * function.
     * 
     * @return An LDLinkedList of Account objects.
     */
    ...
    /**
     * This function returns an LDLinkedList of blocked accounts.
     * 
     * @return An LDLinkedList of blocked accounts.
     */
    public LDLinkedList<Account> getBlockedAccounts(){
        return blockedAccounts;
    }
    /**
     * This function returns an LDLinkedList of history strings.
     * 
     * @return An LDLinkedList of strings of history.
     */
    ...
    public void setPosts(Post post, int process){
        if(process == 0){//add
            if(posts.add(post) == false)
                System.out.println("An error occured when adding post to the LDLinkedList.");
        }
        else if(process == 1){//remove
            if(posts.myremove(post) == false)
                System.out.println("This post in not in the LDLinkedList.");
        }
    }
    /**
    ...
    public void setInbox(Message message, int process){
        if(process == 0){//add
            ...
            if(inbox.myremove(message) == false)
                System.out.println("This post in not in the LDLinkedList.");
        }
    }
    /**
    ...
    public void setOutbox(Message message, int process){
        if(process == 0){//add
            ...
            if(outbox.myremove(message) == false)
                System.out.println("This post in not in the LDLinkedList.");
        }
    }
    /**
    ...
    public void setFollowing(Account account, int process){
        if(process == 0){//add
            ...
            if(following.myremove(account) == false)
                System.out.println("This post in not in the LDLinkedList.");
        }
    }
    /**
    ...
    public void setFollowers(Account account, int process){
        if(process == 0){//add
            ...
            if(followers.myremove(account) == false)
                System.out.println("This post in not in the LDLinkedList.");
        }
    } 
    /**
    ...
    public void setBlockedAccounts(Account account, int process){
        if(process == 0){//add
            ...
            if(blockedAccounts.myremove(account) == false)
                System.out.println("This post in not in the LDLinkedList.");
        }
    }
    /**
    ...
    public void setHistory(String activity,int process){
        if(process == 0){//add
            ...
            if(history.myremove(activity) == false)
                System.out.println("This post in not in the LDLinkedList.");
        }
    }
    ...
    public void viewProfile(Account account){
        if(getIsLogin()){
            if(!isBlocked(account)){
                LDLinkedList<Account> following = account.getFollowing();
                LDLinkedList<Account> followers = account.getFollowers();
    ...
    public void viewPosts(Account account){
        if(getCurrentAccount() == account.getAccountID()){
            LDLinkedList<Post> userPosts = account.getPosts();
            if(userPosts.size() > 0){
    ...
    public void viewInteractions(Account account, LDLinkedList<Account> accounts){
        if((getCurrentAccount() == account.getAccountID()) && getIsPostAreShown()){
            ...
            for(int i=0; i<userPosts.size(); i++){
                ...
                LDLinkedList<Like>    likes = userPosts.get(i).getLikes();
                LDLinkedList<Comment> comments = userPosts.get(i).getComments();
    ...
    public void like(int postID, Account account){
        if(isPostsAreShown){
            LDLinkedList<Post> tempPosts = account.getPosts();
    ...
    public void checkInbox(){
        if(isLogin){
            LDLinkedList<Message> temporary = getInbox();
            if(temporary.size() > 0){
                System.out.println("----------------------------");
    ...
    public void checkOutbox(){
        if(isLogin){
            LDLinkedList<Message> temporary = getOutbox();
            if(temporary.size() > 0){
                System.out.println("----------------------------");
                for(int i=0; i<temporary.size(); i++){
                    System.out.printf("Message ID: %d\n",temporary.get(i).getMessageID());
    ...
    public boolean checkIsFollowing(Account account){
        LDLinkedList<Account> temp = getFollowing();
    ...
    public boolean checkIsFollower(Account account){
        LDLinkedList<Account> temp = getFollowers();
        for(int i=0; i<temp.size(); i++){
            if(temp.get(i).getAccountID() == account.getAccountID())
                return true;
        }
    ...
    public boolean isBlocked(Account account){
        LDLinkedList<Account> blockedAccounts = account.getBlockedAccounts();
     ...
     * @return A boolean value.
     */
    public boolean checkAnyLogin(LDLinkedList<Account> accounts){
     ...
     * @param account The account of the user who wants to unlike a post.
     */
    public void unlike(int postID, Account account){
        if(isPostsAreShown){
            ...
            for(int i=0; i<tempPosts.size(); i++){
                if(tempPosts.get(i).getPostID() == postID){
                    LDLinkedList<Like> tempLikes = tempPosts.get(i).getLikes();
    ...
    /**
     * The function removes a comment made by the current account on a specific post by another
     * account.
     * 
     * @param postID an integer representing the ID of the post that the comment belongs to.
     * @param account An object of the Account class representing the account of the user who wants to
     * uncomment a comment on a post.
     * @param interactionID The interactionID parameter is an identifier for a specific interaction
     * (comment) on a post. It is used to locate the comment that needs to be uncommented.
     */
    public void uncomment(int postID, Account account,int interactionID){
        if(isPostsAreShown){
            ...
            for(int i=0; i<tempPosts.size(); i++){
                if(tempPosts.get(i).getPostID() == postID){
                    LDLinkedList<Comment> tempComments = tempPosts.get(i).getComments();
     ...
     * user account in a system. It is used as an input to the "unblock" method to unblock the
     * specified account.
     */
    ...
    /**
     * The function shows the user's history of actions if they are logged in, otherwise it prompts
     * them to log in first.
     */
    public void showHistory(){
        if(isLogin){
            LDLinkedList<String> tempHistory = this.getHistory();
